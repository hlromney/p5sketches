// Layered image sketch: hue drift, fade-in from grey, and parallax.
...
const PARALLAX_EASE = 0.08;   // 0.01 = floaty and slow, 0.2 = quick and snappy
...
const USE_TILT = true;        // on phones, tilt the device to move the layers
const TILT_RANGE = 3;        // degrees of tilt for full movement (smaller = more sensitive)
...
// Parallax input: target is where the pointer/tilt says to go, pos eases toward it
let target = { x: 0, y: 0 };
let pos = { x: 0, y: 0 };
...
function setupParallaxInput() {
  ...
  // Same approach as your working sketch: a mobile viewport tag, and touch
  // handlers on the canvas itself that stop the page from scrolling
  createMetaTag();
  const c = canvasEl();
  ...
  document.addEventListener('mouseleave', () => {
    if (!tiltActive) { target.x = 0; target.y = 0; }
  });

  if (!USE_TILT || typeof DeviceOrientationEvent === 'undefined') return;

  if (typeof DeviceOrientationEvent.requestPermission === 'function') {
    // iPhone/iPad: motion access must be requested from a tap
    //showTiltPrompt(true);
    const ask = () => {
...
// Tilt, corrected for portrait/landscape. "Neutral" is however the phone
// is held when tilt starts, and it slowly re-centers if you change your grip.
let tiltBase = null;
function onTilt(e) {
  ...
  if (tiltBase === null) tiltBase = { x, y };
  tiltBase.x = lerp(tiltBase.x, x, 0.005);
  tiltBase.y = lerp(tiltBase.y, y, 0.005);

  tiltActive = true;
  if (isTouching || mouseIsPressed) return;
  target.x = constrain((x - tiltBase.x) / TILT_RANGE, -1, 1);
  target.y = constrain((y - tiltBase.y) / TILT_RANGE, -1, 1);
}

let tiltPrompt;
function showTiltPrompt(show) {
  if (!tiltPrompt) {
    tiltPrompt = createDiv('Tap to enable tilt');
    tiltPrompt.style('position', 'fixed');
    tiltPrompt.style('left', '50%');
    tiltPrompt.style('bottom', 'calc(24px + env(safe-area-inset-bottom, 0px))');
    tiltPrompt.style('transform', 'translateX(-50%)');
    tiltPrompt.style('padding', '8px 16px');
    tiltPrompt.style('border-radius', '999px');
    tiltPrompt.style('background', 'rgba(0,0,0,0.55)');
    tiltPrompt.style('color', '#fff');
    tiltPrompt.style('font', '15px sans-serif');
    tiltPrompt.style('pointer-events', 'none');
  }
  tiltPrompt.style('display', show ? 'block' : 'none');
}
...
function computeLayout() {
  ...
  let room = 1;
  if (KEEP_FULL_IMAGE && !reduceMotion) {
    const maxDepth = max(LAYERS.map(l => l.depth ?? 0));
